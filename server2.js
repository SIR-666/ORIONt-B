...
app.post("/createPO", async (req, res) => {
  ...
  try {
    ...
    const existingOrderQuery = `
      SELECT id FROM ProductionOrder_2 WHERE id = @po
    `;
    ...
    const insertOrderQuery = `
      INSERT INTO [dbo].[ProductionOrder_2] 
      (
  ...
  try {
    ...
    while (idExists) {
      const idCheckResult = await pool
        ...
        .query("SELECT 1 FROM [dbo].[ProductionOrder_2] WHERE id = @id;");
    ...
    if (groupSelection && Object.keys(groupSelection).length > 1) {
      ...
      for (const order of splitOrders) {
        insertOrderQuery = await pool
          ...
          .query(`INSERT INTO [dbo].[ProductionOrder_2] 
        (
          id, product_id, qty, date_start, date_end, status, created_at, updated_at, actual_start, actual_end, plant, line, completion_count, [group]
        )
        VALUES (
...
app.get("/getAllPOLine/:line", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("line", sql.VarChar, line)
      .query(
        `SELECT id, status, actual_start, actual_end FROM ProductionOrder_2 WHERE line = @line;`
      );
...
app.get("/getOrders", async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query(`SELECT id FROM ProductionOrder_2;`);
...
app.post("/updatePO", async (req, res) => {
  ...
  try {
    ...
    const statusResult = await pool
      ...
      .input("id", sql.BigInt, id)
      .query(
        `SELECT status, date_start, date_end, completion_count, product_id, actual_start, line, qty, plant FROM [dbo].[ProductionOrder_2] WHERE id = @id`
      );
    ...
    if (currentStatus === "New") {
      const result = await pool
        ...
        .input("group", sql.Int, groupId)
        .query(`UPDATE [dbo].[ProductionOrder_2]
    ...
    const result = await pool
      ...
      .query(
        `WITH UniqueEntries AS (
            ...
            FROM dbo.tb_reasonDowntime_2 rd
            JOIN dbo.tb_filling_downtime_all2_2 fd
...
app.post("/getDowntime", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("end", sql.DateTime, date_end)
      .input("line", sql.VarChar, line)
      .query(`SELECT Date, Line, Downtime_Category, Minutes FROM dbo.tb_reasonDowntime_2 
    WHERE Date >= @start 
    AND Date < @end
    AND Line = @line;`);
...
app.get("/getDowntimeId/:id", async (req, res) => {
  ...
  try {
    ...
    const query = `SELECT id, Date, Downtime_Category, Mesin, Jenis, Keterangan FROM dbo.tb_reasonDowntime_2 where id = @id;`;
...
app.get("/getAllPerformance", async (req, res) => {
  try {
    ...
    const query = `
      ...
      FROM dbo.tb_filling_downtime_all2_2
      WHERE No LIKE '_E___'
  ...
  try {
    ...
    const overlapCheck = await pool
      ...
      .input("line", sql.VarChar, line).query(`
        SELECT * FROM dbo.tb_reasonDowntime_2
        WHERE 
          Date < @newEndTime
    ...
    const resultReason = await transaction
      ...
      .query(`INSERT INTO dbo.tb_reasonDowntime_2 
              (Unit
              ,Code
              ,Date
              ,Month
    ...
    const existingEntry = await transaction
      ...
      .input("No", sql.VarChar, `${table2Data.line}%`).query(`
              SELECT No, Week, Tanggal, Downtime, TypeDowntime FROM dbo.tb_filling_downtime_all2_2
    ...
    if (existingEntry.recordset && existingEntry.recordset.length > 0) {
      ...
      await transaction
        ...
        .input("id", sql.VarChar, `${table2Data.id}`).query(`
                  UPDATE dbo.tb_filling_downtime_all2_2
...
app.put("/updateStoppage", async (req, res) => {
  ...
  try {
    ...
    const statusResult = await pool
      ...
      .input("id", sql.Int, id)
      .query(
        `SELECT id, Minutes FROM [dbo].[tb_reasonDowntime_2] WHERE id = @id`
      );
    ...
    const resultReason = await transaction
      ...
      .input("comments", sql.VarChar, comments)
      .input("duration", sql.Int, duration)
      .query(`UPDATE dbo.tb_reasonDowntime_2 
              SET Date = @date_start, 
...
app.post("/deleteStoppage", async (req, res) => {
  ...
  try {
    ...
    const statusResult = await pool
      .request()
      .input("id", sql.Int, id)
      .query(`SELECT * FROM [dbo].[tb_reasonDowntime_2] WHERE id = @id`);
    ...
    if (statusResult.recordset.length > 0) {
      ...
      const result = await pool
        .request()
        .input("id", sql.Int, id)
        .query(`DELETE FROM [dbo].[tb_reasonDowntime_2] WHERE id = @id`);
  ...
  try {
    ...
    if (existingEntry.recordset && existingEntry.recordset.length > 0) {
      result = await pool
        ...
        .input("DateOnly", sql.DateTime, parsedDateStart)
        .input("No", sql.VarChar, `${parsedLine.line}%`)
        .query(`UPDATE dbo.tb_filling_downtime_all2_2
      ...
      result = await pool
        ...
        .query(`INSERT INTO dbo.tb_filling_downtime_all2_2 (
                ID
                ,No
                ,Week
                ,Week2
    ...
    const upsertData = async (type, value) => {
      ...
      const existingEntry = await pool
        ...
        .input("TypeDowntime", sql.VarChar, `%${type}%`).query(`
          SELECT Downtime FROM dbo.tb_filling_downtime_all2_2
          WHERE Tanggal = @DateOnly
...
app.post("/deleteSpeedLoss", async (req, res) => {
  ...
  try {
    ...
    const statusResult = await pool
      ...
      .query(
        `SELECT * FROM [dbo].[tb_filling_downtime_all2_2] WHERE Tanggal = @date AND TypeDowntime LIKE '%SPEED%' AND No LIKE @line`
      );
    ...
    if (statusResult.recordset.length > 0) {
      const result = await pool
        ...
        .input("line", sql.VarChar, `${lineInitial}E%`)
        .query(
          `DELETE FROM [dbo].[tb_filling_downtime_all2_2] WHERE Tanggal = @date AND TypeDowntime LIKE '%SPEED%' AND No LIKE @line`
        );
      console.log("Delete result:", result);
      const rowsAffected = result.rowsAffected[0] || 0;
...
app.post("/getQualityLoss", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .query(`SELECT Downtime FROM dbo.tb_filling_downtime_all2_2 
...
app.post("/getNominalSpeed", async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("start", sql.DateTime, parsedDateStart)
      .input("end", sql.DateTime, parsedDateEnd)
      .query(`SELECT Tanggal, Downtime FROM dbo.tb_filling_downtime_all2_2 
