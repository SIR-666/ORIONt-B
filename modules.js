...
function parseLine(line, date_start, week){
...
const saveSplitOrders = async (pool, poNumber, product_id, qty, startDate, endDate, startTime, endTime, plant, line, groupSelections) => {
  try {
    const shifts = [
    ...
    const allShifts = [...shifts, ...shifts.map(shift => ({ ...shift, isPreviousDay: true }))];
    ...
    if (currentShiftEnd) {
      await pool
        ...
        .input("shiftEnd", sql.DateTime, currentShiftEnd)
        .query(`
      ...
      if (shiftResult.recordset.length > 0) {
          ...
          for (const record of shiftResult.recordset) {
            ...
            if (dbStartTime > dateObj && dbStartTime < endTime) {
                if (earliestAfterCurrentTime === null || dbStartTime < earliestAfterCurrentTime) {
          ...
          if (earliestAfterCurrentTime !== null) {
              ...
              console.log("Updated endTime to the start of the next production order:", endTime);
          }
        }
...
const getOrCreateProduct = async (pool, material) => {
  ...
  `;
  const productResult = await pool.request().input("sku", sql.VarChar, material).query(productQuery);
  ...
  const insertProductQuery = `
    ...
    SELECT @newId AS id;
  `;
  const insertResult = await pool.request().input("sku", sql.VarChar, material).query(insertProductQuery);
...
module.exports = { parseTableFillingValues, parseLine, parseLineDowntime, saveSplitOrders, getShift, getShiftEndTime, formatDateTime, getOrCreateProduct };